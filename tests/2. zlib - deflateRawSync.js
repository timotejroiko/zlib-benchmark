...
while(performance.now() < warmup + 2000) {
	...
	const c = zlib.deflateRawSync(data, args);
	const d = zlib.inflateRawSync(c, args);
...
console.log(`DeflateRawSync x ${avg1.toFixed(2)} ops/sec ± ${dev1.toFixed(2)} (${(avg4 * avg1 / 1024 / 1024).toFixed(3)} MB/s)`);
console.log(`InflateRawSync x ${avg2.toFixed(2)} ops/sec ± ${dev2.toFixed(2)} (${(avg3 * avg2 / 1024 / 1024).toFixed(3)} MB/s)`);
