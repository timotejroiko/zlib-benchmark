...
const compress = zlib("deflate",Object.assign(args,{flush:zlib.constants.Z_SYNC_FLUSH_UNSAFE}));
const decompress = zlib("inflate",Object.assign(args,{flush:zlib.constants.Z_SYNC_FLUSH_UNSAFE}));

let warmup = performance.now();
while(performance.now() < warmup + 2000) {
	let data = Buffer.from(functions.randomize(dat));
	let c = compress(data);
	let d = decompress(c);
	if(d.toString() !== data.toString()) { throw "data validation failed"; }
}
...
console.log(`Deflate (unsafe) x ${avg1.toFixed(2)} ops/sec ± ${dev1.toFixed(2)} (${(avg4 * avg1 / 1024 / 1024).toFixed(3)} MB/s)`);
console.log(`Inflate (unsafe) x ${avg2.toFixed(2)} ops/sec ± ${dev2.toFixed(2)} (${(avg3 * avg2 / 1024 / 1024).toFixed(3)} MB/s)`);
