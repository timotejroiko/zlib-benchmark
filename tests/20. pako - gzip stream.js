...
const compress = new pako.Deflate(Object.assign(args,{gzip:true}));
const decompress = new pako.Inflate(args);

let warmup = performance.now();
while(performance.now() < warmup + 2000) {
	let data = functions.randomize(dat);
	compress.push(data,pako.Z_SYNC_FLUSH);
	let c = compress.result;
	decompress.push(c,pako.Z_SYNC_FLUSH)
	let d = decompress.result;
	if(decoder.decode(d) !== data) { throw "data validation failed"; }
}
...
console.log(`Inflate stream x ${avg2.toFixed(2)} ops/sec ± ${dev2.toFixed(2)} (${(avg3 * avg2 / 1024 / 1024).toFixed(3)} MB/s)`);
